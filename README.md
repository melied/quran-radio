...
- تعدد اللغات: واجهة عربية/English بزر تبديل فوري (الاتجاه RTL/LTR يتكيف تلقائيًا)
- موثوقية: عند تعذر بث إذاعة ينتقل تلقائيًا إلى بقية الإذاعات قبل إعلان الانقطاع
- نسخة ووردبريس: إضافة شورت كود [quran_radio] جاهزة في مجلد wordpress/
- إضافة متصفح: Chrome/Edge (MV3) — تشغيل مستمر من أي صفحة عبر Offscreen، اختصار Ctrl+Shift+U، وشارة ON AIR
...
نسخة إضافة MV3 في مجلد `extension/` — تشغيل الإذاعة **من أي صفحة** بأيقونة دائمة في شريط الأدوات:

- **تشغيل مستمر**: الصوت يعيش في وثيقة Offscreen خفية — يكمل حتى بعد إغلاق النافذة المنبثقة، مع تحكم من شاشة القفل.
- **اختصار عام**: Ctrl+Shift+U (قابل للتغيير من chrome://extensions/shortcuts) يشغّل/يوقف من أي مكان.
- **شارة ON AIR**: أيقونة المتصفح تُظهر حالة التشغيل من أي تبويب.
- **مؤقّت النوم**: يكمل العدّ حتى مع إغلاق النافذة — أدق من نسخة الويب.
- **تعدد اللغات**: نفس زر ع/EN مع انقلاب الاتجاه تلقائيًا.

**التثبيت المحلي (Load unpacked):**

1. حمّل مجلد `extension/` من المستودع (Code ← Download ZIP ثم فك الضغط).
2. افتح `chrome://extensions` في كروم أو `edge://extensions` في إيدج.
3. فعّل **Developer mode** ثم اضغط **Load unpacked** واختر مجلد `extension`.
4. اضغط أيقونة الإذاعة في شريط الأدوات — والاختصار Ctrl+Shift+U يشغّل/يوقف من أي صفحة.

> **النشر على المتاجر:** متجر Chrome يتطلب حساب مطوّر (5$ مرة واحدة) وحزم المجلد + لقطات شاشة؛ متجر Edge مجاني ونفس الحزمة تعمل مباشرة؛ Firefox يحتاج تعديل طبقة الخلفية (لا يوجد offscreen API). الإضافة لا تجمع أي بيانات — التفضيلات محلية فقط.

## نسخة ووردبريس

أضف الإذاعة إلى أي موقع ووردبريس عبر إضافة الشورت كود المرفقة في `wordpress/quran-radio-widget.php`:

1. حمّل الملف وضعه في `wp-content/plugins/quran-radio-widget/` ثم فعّل الإضافة من لوحة ووردبريس.
2. ضع الشورت كود في أي صفحة أو ودجت:

```
[quran_radio]
```

خيارات الشورت كود: `station` (رقم الإذاعة من 0)، `height` (ارتفاع المشغل)، مثال: `[quran_radio station="3" height="460"]`.

> في وضع اللغة الإنجليزية تبقى أسماء الإذاعات بعربية (أسماء علم) وبقية الواجهة تتترجم.
